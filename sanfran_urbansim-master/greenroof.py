...
def NoInterventionGreenRoof():
    ...
    for(all buildings in the table)
        ...
        #bringing in UrbanSim native variables        
        income_dict.update({index : some_get_function})
        employment_dict.update({index : some_get_function})
        employment_density_dict.update({index : some_get_function})
        property_value_dict.update({index : some_get_function})
        vacant_dict.update({index : some_get_function})
        stories_dict.update({index : some_get_function})
...
def SanFranGreenRoof():
    ...
    for(all buildings in the table):
        ...
        if  not residential: #This set of conditions only applies when the mandate applies
...
def TorontoGreenRoof(): 
    ...
    for(all buildings in the table):
        ...
        if  not residential or (residential and number_stories > 6): #This set of conditions only applies when the mandate applies
            green_roof = 1
            mandate = 1
